...
public class KeycloakAdminClient implements openerp.openerpresourceserver.service.KeycloakAdminClient {
    ...
    @Override
    public void createUserInKeycloak(EmployeeRequest employeeRequest) {
        ...
        user.setFirstName(employeeRequest.getName());
        ...
        String userId = usersResource.search(employeeRequest.getUsername()).get(0).getId();
        ...
        RoleRepresentation clientRole = realmResource.clients()
                .get(clientUuid)
                .roles()
                .get(employeeRequest.getRole().toKeycloakRole()) // Sử dụng ánh xạ
                .toRepresentation();
    ...
    @Override
    public void updateUserInKeycloak(EmployeeRequest employeeRequest) {
        Keycloak keycloak = getKeycloakInstance();
        RealmResource realmResource = keycloak.realm(realm);
        UsersResource usersResource = realmResource.users();

        // Tra cứu user trong Keycloak bằng username
        List<UserRepresentation> users = usersResource.search(employeeRequest.getUsername(), 0, 1);
        if (users.isEmpty()) {
            throw new RuntimeException("Không tìm thấy người dùng trong Keycloak với username: " + employeeRequest.getUsername());
        }
